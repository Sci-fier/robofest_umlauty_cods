...
//функция подсчета угла наклона манипулятора
int calc_angel(int x){
  x = map(x, 345, 908, 0, 1023); // преобразования значения из диапазона (345,908) в (0,1023)
  // программное ограничения чисел, выходящих из границ диапазона
  if(x < 0){x = 0;} 
  if(x > 1023){x = 1023;}
  return(x); // возвращения результата
}
...
void loop () {
  ...
  data[5] = calc_angel(analogRead(PIN_ARM)); // подсчет угла манипулятора
