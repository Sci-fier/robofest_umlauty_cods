...
void arm_move(int pos){
  ...
  int shift = pos - analogRead(PIN_POT);// создание переменной, которая указывает необходимое смещение для манипулятора
  shift = map(shift, 120, -120, 0, 180);//перевод полченных значения из диапазона (-120,120) в диапазон (0,180)
  //ограничение диапазона
  if(shift<0){shift=0;}
  if(shift>180){shift=180;} 
  servo_arm.write(shift);// отправка итогового значения смещения манипулятора
}
...
void loop() {
  ...
  // в случае отсутствия обратной связи все моторы клешни и основания останавливаются
  else{
    robot_move(255, 255);
    servo_claw.write(90); 
  }
