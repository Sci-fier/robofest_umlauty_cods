...
//функция преобразования входящего значения в движения манипулятора
void arm_move(int pos){
  pos = map(pos, 0, 1023, 500, 200);// перевод значений, полученных с потенциометра на пульте, из диапазона (0,1023) в диапазон (500,200)
  int pos_new = pos - analogRead(PIN_POT);// создание переменной, которая указвает цель для манипулятора
  pos_new = map(pos_new, 120, -120, 0, 180);//перевод полченных значений из диапазона (-120,120) в диапазон (0,180)
  //ограничение диапазона
  if(pos_new<0){pos_new=0;}
  if(pos_new>180){pos_new=180;} 
  servo_arm.write(pos_new);// отправка итогового значения для следования манипулятору  
}
...
void loop() {
  ...
  // в случае отсутствия обратной связи моторы останавливаются, а клешня переходит в начальное состояние
  else{
    robot_move(255, 255);
    servo_claw.write(90); 
  }
