...
#define PIN_IN1R 3 // INPUT 1 RIGHT
#define PIN_IN2R 15 // INPUT 2 RIGHT
#define PIN_POT 14 // PIN POT

//первичная настройка радиомодуля
RF24 radio(7, 8); // порты D7, D8: CE (SS - выбор ведомого)
const uint32_t pipe = 111156789; // адрес рабочей трубы
long int data[7]; // массив передачи
Servo servox; // создание переменной, отвечающей за плавное движение по х класса ServoSmooth
Servo servoy; // создание переменной, отвечающей за плавное движение по у класса ServoSmooth
...
void arm_move(int pos){
  pos = map(pos, 0, 1023, 500, 200);
  Serial.print(pos);
  Serial.print(' ');
  Serial.print(analogRead(PIN_POT));
  Serial.print(' ');
  int q = pos - analogRead(PIN_POT);
  Serial.print(q);
  Serial.print(' ');
  q = map(q, 120, -120, 0, 180);
  if(q<0){q=0;}
  if(q>180){q=180;} 
  Serial.print(q);
  Serial.print(' ');
...
void setup() {
  ...
  servo_claw.attach(PIN_SC, 1000, 2000); // резервирование порта PIN_SA под servo_cla
}
...
void loop() {
  if (radio.available()){
    ...
    servox.write(data[0]); // задаем целевой угол (угол в градусах) для оси Y
    servoy.write(data[1]); // задаем целевой угол (угол в градусах) для оси Х
    robot_move(data[2], data[3]); // движение робота на координаты data[2], data[3]
    servo_claw.write(data[4] * 90); // задаём вращение клешни
    arm_move(data[5]); // задаём положение манипулятора
#ifdef DEBUG
